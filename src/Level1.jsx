...
// Pre-allocate geometry and materials outside the render loop to prevent memory overhead
const sharedConeGeometry = new THREE.ConeGeometry(0.4, 1, 4);
const sharedRedMaterial = new THREE.MeshBasicMaterial({ color: "#ff0000" });
const sharedOrangeMaterial = new THREE.MeshBasicMaterial({ color: "#ff8800" });
...
const FirePit = ({ startX, endX, y, z }) => {
  ...
  useFrame(({ clock }) => {
    ...
    if (fireGroupRef.current) {
      ...
      // Use standard for loop instead of .forEach() to prevent closure allocations and reduce GC pressure
  ...
  // Added an epsilon to prevent IEEE 754 precision issues when incrementing by 0.5
  for (let i = startX; i <= endX + 0.0001; i += 0.5) {
    ...
    flames.push(
      <mesh
        ...
        material={isRed ? sharedRedMaterial : sharedOrangeMaterial}
        dispose={null}
      />
    );
  }
