...
// Pre-allocate geometries and materials to avoid recreation in the FirePit loop
const sharedConeGeometry = new ConeGeometry(0.4, 1, 4);
const redMaterial = new MeshBasicMaterial({ color: "#ff0000" });
const orangeMaterial = new MeshBasicMaterial({ color: "#ff8800" });
...
const FirePit = ({ startX, endX, y, z }) => {
  ...
  useFrame(({ clock }) => {
    ...
    if (fireGroupRef.current) {
      ...
      // Use traditional for loop instead of .forEach to avoid per-frame closure allocation
      for (let i = 0; i < children.length; i++) {
        const flame = children[i];
        // Rapid scaling and slight chaotic math for retro fire effect using secure random
        const scaleY = 1 + Math.sin(t * 20 + i * 10) * 0.5 + getSecureRandom() * 0.2;
  ...
  const flames = [];
  // Place multiple cones across the gap. Add epsilon to handle floating point precision
  ...
  for (let i = startX; i <= endX + epsilon; i += 0.5) {
    ...
    flames.push(
      <mesh
        ...
        dispose={null} // Prevent R3F from auto-disposing shared geometries/materials
      />
    );
  }
