...
// Pre-allocate geometries and materials to avoid GC pressure
...
const FirePit = ({ startX, endX, y, z }) => {
  ...
  useFrame(({ clock }) => {
    ...
    if (fireGroupRef.current) {
      ...
      // Using a standard for loop to avoid GC pressure from inline functions on every frame
  ...
  for (let i = startX; i <= endX; i += 0.5) {
    // 0.5 increments do not suffer from float precision issues
    const isRed = i % 1 === 0;
    flames.push(
      <mesh
        key={i}
