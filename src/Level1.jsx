...
// Cache for geometries and materials to avoid recreation and reduce memory overhead
...
const Block = ({ position, color = "#4a4a4a", isObstacle = true, isFloor = true, args = [1, 1, 1] }) => {
  const geomKey = args.join(',');
  if (!geometryCache.has(geomKey)) {
    geometryCache.set(geomKey, new THREE.BoxGeometry(...args));
  }
  ...
  return (
    <mesh
      ...
      geometry={geometryCache.get(geomKey)}
      material={materialCache.get(color)}
      dispose={null}
    />
  );
};
...
const redFireMaterial = new THREE.MeshBasicMaterial({ color: "#ff0000" });
const orangeFireMaterial = new THREE.MeshBasicMaterial({ color: "#ff8800" });
...
const FirePit = ({ startX, endX, y, z }) => {
  ...
  useFrame(({ clock }) => {
    ...
    if (fireGroupRef.current) {
      ...
      // Use standard for-loop to avoid per-frame closure allocations
  ...
  for (let i = startX; i <= endX; i += 0.5) {
    ...
    flames.push(
      <mesh
        ...
        material={isRed ? redFireMaterial : orangeFireMaterial}
        dispose={null}
      />
    );
  }
