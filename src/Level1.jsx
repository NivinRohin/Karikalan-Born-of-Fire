...
const Block = ({ position, color = "#4a4a4a", isObstacle = true, isFloor = true, args = [1, 1, 1] }) => {
  ...
  if (!blockGeometryCache.has(geoKey)) {
    blockGeometryCache.set(geoKey, new THREE.BoxGeometry(...args));
  }

  if (!blockMaterialCache.has(color)) {
    blockMaterialCache.set(color, new THREE.MeshStandardMaterial({ color, roughness: 1 }));
  }
  ...
  return (
    <mesh
      ...
      dispose={null}
      geometry={blockGeometryCache.get(geoKey)}
      material={blockMaterialCache.get(color)}
    >
    </mesh>
  );
};

// Pre-allocate geometry and materials for FirePit
const firePitConeGeo = new THREE.ConeGeometry(0.4, 1, 4);
const firePitMatRed = new THREE.MeshBasicMaterial({ color: "#ff0000" });
const firePitMatOrange = new THREE.MeshBasicMaterial({ color: "#ff8800" });
  ...
  for (let i = startX; i <= endX + 0.0001; i += 0.5) {
    ...
    flames.push(
      <mesh
        ...
        dispose={null}
        geometry={firePitConeGeo}
        material={isRed ? firePitMatRed : firePitMatOrange}
      >
      </mesh>
    );
  }
