...
function getGeometry(argsKey, args) {
  if (!geometryCache.has(argsKey)) {
    geometryCache.set(argsKey, new THREE.BoxGeometry(...args));
  }
  return geometryCache.get(argsKey);
}
...
const Block = ({ position, color = "#4a4a4a", isObstacle = true, isFloor = true, args = [1, 1, 1] }) => {
  ...
  // Memoizing the retrieval so it doesn't run the functions every render,
  // although Map.get is fast enough anyway.
  const geometry = useMemo(() => getGeometry(argsKey, args), [argsKey, args]);
...
const fireGeom = new THREE.ConeGeometry(0.4, 1, 4);
const fireMatRed = new THREE.MeshBasicMaterial({ color: "#ff0000" });
const fireMatOrange = new THREE.MeshBasicMaterial({ color: "#ff8800" });
...
const FirePit = ({ startX, endX, y, z }) => {
  ...
  useFrame(({ clock }) => {
    ...
    if (fireGroupRef.current) {
      // Use traditional for loop as per memory to reduce garbage collection pressure
  ...
  const flames = [];
  // Use small epsilon as per memory to handle IEEE 754 precision
  ...
  for (let i = startX; i <= endX + epsilon; i += 0.5) {
    ...
    flames.push(
      <mesh
        ...
        geometry={fireGeom}
        material={isRed ? fireMatRed : fireMatOrange}
        dispose={null}
      />
    );
  }
