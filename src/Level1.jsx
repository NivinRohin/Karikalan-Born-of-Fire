...
// Pre-allocate geometries and materials for FirePit to avoid creating them on every render
...
const FirePit = ({ startX, endX, y, z }) => {
  ...
  useFrame(({ clock }) => {
    ...
    if (fireGroupRef.current) {
      ...
      // Use traditional for loop instead of forEach for better performance in high-frequency useFrame
  ...
  for (let i = startX; i <= endX; i += 0.5) {
    ...
    flames.push(
      <mesh
        ...
        dispose={null} // Prevent R3F from disposing shared geometry/materials
      />
    );
  }
