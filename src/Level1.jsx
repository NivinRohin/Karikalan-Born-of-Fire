...
const FirePit = ({ startX, endX, y, z }) => {
  ...
  useFrame(({ clock }) => {
    ...
    if (fireGroupRef.current) {
      // ⚡ Bolt: Using standard `for` loop instead of `.forEach` to prevent allocating inline callback functions
      // on every frame, reducing Garbage Collection (GC) pressure and avoiding micro-stutters.
