...
// Pre-allocate geometry and materials to prevent continuous object creation
...
const FirePit = ({ startX, endX, y, z }) => {
  ...
  useFrame(({ clock }) => {
    ...
    if (fireGroupRef.current) {
      // Use standard for loop instead of .forEach to reduce function allocations and GC
  ...
  // Using an epsilon for float comparison safety
  for (let i = startX; i <= endX + 0.0001; i += 0.5) {
    ...
    flames.push(
      <mesh
        ...
        dispose={null} // Prevent automatic disposal since we're using shared resources
      />
    );
  }
