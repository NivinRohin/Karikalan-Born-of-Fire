...
import { BoxGeometry, MeshStandardMaterial, ConeGeometry, MeshBasicMaterial } from 'three';

// Caches for Block to prevent recreating geometries and materials
...
const Block = ({ position, color = "#4a4a4a", isObstacle = true, isFloor = true, args = [1, 1, 1] }) => {
  const geometryKey = args.join(',');
  if (!boxGeometries.has(geometryKey)) {
    boxGeometries.set(geometryKey, new BoxGeometry(...args));
  }

  if (!blockMaterials.has(color)) {
    blockMaterials.set(color, new MeshStandardMaterial({ color, roughness: 1 }));
  }

  return (
    <mesh position={position} userData={{ isObstacle, isFloor }} dispose={null} geometry={boxGeometries.get(geometryKey)} material={blockMaterials.get(color)}>
      {/*
        Geometry and material are now cached and passed via props.
        dispose={null} prevents auto disposal on unmount since they are shared.
      */}
    </mesh>
  );
};

// Pre-allocate geometries and materials for FirePit
const fireConeGeometry = new ConeGeometry(0.4, 1, 4);
const fireRedMaterial = new MeshBasicMaterial({ color: "#ff0000" });
const fireOrangeMaterial = new MeshBasicMaterial({ color: "#ff8800" });
...
const FirePit = ({ startX, endX, y, z }) => {
  ...
  useFrame(({ clock }) => {
    ...
    if (fireGroupRef.current) {
      ...
      // Use standard for loop to reduce GC pressure by avoiding inline function allocation
