...
// Caches for optimizing Block instantiations
...
const Block = ({ position, color = "#4a4a4a", isObstacle = true, isFloor = true, args = [1, 1, 1] }) => {
  ...
  return (
    <mesh
      ...
      geometry={geometryCache.get(argsKey)}
      material={materialCache.get(color)}
      dispose={null} // Prevent automatic disposal of shared objects
    />
  );
};

// Pre-allocate FirePit geometries and materials outside component
...
const FirePit = ({ startX, endX, y, z }) => {
  ...
  useFrame(({ clock }) => {
    ...
    if (fireGroupRef.current) {
      // Use standard for loop to avoid per-frame closure allocations in .forEach()
