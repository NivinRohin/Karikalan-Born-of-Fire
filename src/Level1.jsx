...
const Block = ({ position, color = "#4a4a4a", isObstacle = true, isFloor = true, args = [1, 1, 1] }) => {
  ...
  if (!boxGeometryCache.has(argsKey)) {
    boxGeometryCache.set(argsKey, new THREE.BoxGeometry(...args));
  }

  if (!standardMaterialCache.has(color)) {
    standardMaterialCache.set(color, new THREE.MeshStandardMaterial({ color, roughness: 1 }));
  }

  return (
    <mesh position={position} userData={{ isObstacle, isFloor }} geometry={boxGeometryCache.get(argsKey)} material={standardMaterialCache.get(color)} dispose={null}>
    </mesh>
  );
};

// Caches for FirePit resources
const coneGeometry = new THREE.ConeGeometry(0.4, 1, 4);
...
const FirePit = ({ startX, endX, y, z }) => {
  ...
  for (let i = startX; i <= endX; i += 0.5) {
    const isRed = i % 1 === 0;
    flames.push(
      <mesh key={i} position={[i, y, z]} userData={{ isHazard: true }} geometry={coneGeometry} material={isRed ? redMaterial : orangeMaterial} dispose={null}>
      </mesh>
    );
  }
