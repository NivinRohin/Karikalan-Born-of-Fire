...
// Pre-allocate geometries and materials to reduce GC pressure
...
const FirePit = ({ startX, endX, y, z }) => {
  ...
  useFrame(({ clock }) => {
    ...
    if (fireGroupRef.current) {
      ...
      // Using standard for loop instead of .forEach to reduce GC pressure
  ...
  for (let i = startX; i <= endX; i += 0.5) {
    ...
    flames.push(
      <mesh
        ...
        dispose={null} // Prevent R3F from auto-disposing shared geometries and materials
      />
    );
  }
